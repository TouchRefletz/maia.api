...
export const ImageSlotCard: React.FC<ImageSlotCardProps & { parentGroupId?: string | number }> = ({
  ...
  const runAIExtraction = useCallback(async () => {
    ...
    if (!parentCrops || parentCrops.length === 0) {
      setAiState(prev => ({ ...prev, status: 'error', message: 'Não foi possível obter os bounds da questão' }));
    ...
    try {
      ...
      for (const pageNum of pages) {
        ...
        setAiState(prev => ({ ...prev, status: 'extracting', message: `Analisando página ${pageNum}...` }));
        ...
        if (result.success && result.crops && result.crops.length > 0) {
          ...
          if ((window as any).confirmAISlotDirectly) {
            ...
            setAiState(prev => ({ ...prev, status: 'error', message: 'Função de confirmação não disponível' }));
            resetToStartButtonRef.current = setTimeout(() => setAiState(prev => ({ ...prev, status: 'ready' })), 5000);
            return;
          }
        }
      ...
      setAiState(prev => ({ ...prev, status: 'error', message: 'Nenhuma imagem encontrada na questão' }));
      resetToStartButtonRef.current = setTimeout(() => setAiState(prev => ({ ...prev, status: 'ready' })), 5000);
  ...
  const renderAIHeader = () => {
    ...
    if (isSuccess) { title = aiState.message || "Extração Concluída"; icon = "✅"; }
    if (isError) { title = aiState.message || "Erro na Extração"; icon = "❌"; }
    if (aiState.status === 'cancelled') { title = "Cancelado"; icon = "🛑"; }
    if (aiState.status === 'ready') { title = "Extrair imagem com IA"; icon = "✨"; }
